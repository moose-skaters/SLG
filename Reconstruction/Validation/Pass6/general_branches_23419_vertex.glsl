...
void main(){
  ...
  (u_xlat0.x = (_Main02Angle * 0.017453292));
  (u_xlat1.x = cos(u_xlat0.x));
  (u_xlat0.x = sin(u_xlat0.x));
  (u_xlat0.xy = (u_xlat0.xx * in_TEXCOORD0.yx));
  (u_xlat6.x = ((in_TEXCOORD0.x * u_xlat1.x) + (-u_xlat0.x)));
  (u_xlat6.y = ((in_TEXCOORD0.y * u_xlat1.x) + u_xlat0.y));
  (u_xlat16_2.xy = ((u_xlat6.xy * _MainTex02_ST.xy) + _MainTex02_ST.zw));
  (u_xlat16_2.xy = (u_xlat16_2.xy + vec2(-0.5, -0.5)));
  (u_xlat16_2.xy = ((u_xlat16_2.xy * _main02UVMove.zz) + vec2(0.5, 0.5)));
  (u_xlat0.xy = (_Time.yy * _main02UVMove.xy));
  (u_xlat0.xy = fract(u_xlat0.xy));
  (vs_TEXCOORD0.zw = (u_xlat0.xy + u_xlat16_2.xy));
  (u_xlat16_2.xy = ((in_TEXCOORD0.xy * _MaskTex_ST.xy) + _MaskTex_ST.zw));
  (u_xlat16_2.xy = (u_xlat16_2.xy + vec2(-0.5, -0.5)));
  (u_xlat16_2.xy = ((u_xlat16_2.xy * _maskUVMove.zz) + vec2(0.5, 0.5)));
  (u_xlat0.xy = (_Time.yy * _maskUVMove.xy));
  (u_xlat0.xy = fract(u_xlat0.xy));
  (u_xlat0.xy = (u_xlat0.xy + u_xlat16_2.xy));
  (vs_TEXCOORD1.zw = ((vec2(_MaskTexUV) * in_TEXCOORD3.xy) + u_xlat0.xy));
  ...
  (vs_TEXCOORD5.xyz = (u_xlat0.xyz + hlslcc_mtx4x4unity_ObjectToWorld[3].xyz));
