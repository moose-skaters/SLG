...
layout(location = 6) uniform sampler2D _Splat_Golobal;
layout(location = 7) uniform sampler2D _SparkMap;
layout(location = 8) uniform sampler2D _SpecMaskMap;
...
void main(){
  ...
  (u_xlat2.x = dot(_MainLightColor.xyz, vec3(0.21259999, 0.71520001, 0.0722)));
  (u_xlat13.xyz = ((-u_xlat2.xxx) + _MainLightColor.xyz));
  (u_xlat2.xyz = ((vec3(_LightColorDesat) * u_xlat13.xyz) + u_xlat2.xxx));
  (u_xlat35 = _MainLightPosition.y);
  (u_xlat35 = clamp(u_xlat35, 0.0, 1.0));
  (u_xlat3.xyz = (u_xlat16_19.xyz * vec3(u_xlat35)));
  (u_xlat3.xyz = (u_xlat2.xyz * u_xlat3.xyz));
  (u_xlat16_6.xyz = (u_xlat16_19.xyz * vs_TEXCOORD8.xyz));
  (u_xlat3.xyz = ((u_xlat3.xyz * unity_LightData.zzz) + u_xlat16_6.xyz));
  (u_xlat16_6.x = min(_AdditionalLightsCount.x, unity_LightData.y));
  (u_xlatu35 = uint(int(u_xlat16_6.x)));
  (u_xlat16_6.xyz = u_xlat3.xyz);
  ...
  (u_xlat35 = (_WorldSpaceCameraPos.y + (-_MinHeight)));
  (u_xlat3.x = ((-_MinHeight) + _MaxHeight));
  (u_xlat3.x = (1.0 / float(u_xlat3.x)));
  (u_xlat35 = (u_xlat35 * u_xlat3.x));
  (u_xlat35 = clamp(u_xlat35, 0.0, 1.0));
  (u_xlat16_3.xyz = texture(_Splat_Golobal, vs_TEXCOORD6.xy, _GlobalMipBias.x).xyz);
  (u_xlat16_8.xyz = (u_xlat2.xyz * u_xlat16_3.xyz));
  (u_xlat2.xy = (vs_TEXCOORD7.xy / vs_TEXCOORD7.ww));
  (u_xlat16_24 = texture(_SparkMap, vs_TEXCOORD6.zw, _GlobalMipBias.x).w);
  (u_xlat16_3.xy = texture(_SpecMaskMap, u_xlat2.xy, _GlobalMipBias.x).xy);
  (u_xlat16_9.xyz = (vec3(u_xlat16_24) * _SparkColor.xyz));
  ...
  (u_xlat16_6.xyz = ((vec3(u_xlat35) * u_xlat16_8.xyz) + u_xlat16_6.xyz));
